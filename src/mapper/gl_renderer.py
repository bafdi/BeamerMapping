...
class GLCompositor:
    ...
    def composite(
        ...
        """
        Compositing via GPU - gleiche Signatur wie composite_polygons_fast.
        ...
        try:
            ...
            for image, source_points, output_points in polygons_data:
                ...
                # Source-Punkte in Pixelkoordinaten
                src_pts = np.array([
                    [source_points[i][0] * img_w, source_points[i][1] * img_h]
                    for i in range(4)
                ], dtype=np.float32)

                # Output-Punkte in Pixelkoordinaten
                dst_pts = np.array([
                    [output_points[i][0] * out_w, output_points[i][1] * out_h]
                    for i in range(4)
                ], dtype=np.float32)

                # Homographie berechnen (output -> source = inverse)
                ...
                # Inverse Homography als mat3 (column-major fuer OpenGL)
